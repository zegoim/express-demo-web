...
function initEvent() {
  ...
  zg.on('publishQualityUpdate', (streamId, stats) => {
    ...
    $('#sendVideoCodec').text(stats.video.googCodecName)
  ...
  zg.on('playQualityUpdate', (streamId, stats) => {
      ...
      $('#receiveVideoCodec').text(stats.video.googCodecName)
...
$('#startPublishing').on('click', util.throttle( async function () {
  ...
  this.classList.add('border-primary')
  if(!published) {
      const flag =  await startPublishingStream(id, getCodecConfig());
      if(flag) {
...
$('#startPlaying').on('click', util.throttle( async function () {
  ...
  this.classList.add('border-primary')
  if(!played) {
      const flag =  await startPlayingStream(id, getCodecConfig());
      if(flag) {
